...
public class MatchTask implements CommandLineRunner {
    ...
    public void run(String... args) {
        while (true) {
            Order order = MatchData.queue.poll();
            if (order != null) {
                // 撮合委托订单
                this.match(order);
            }
    ...
    public void match(Order order) {
        ...
        if (order.getStatus() == 2) {
            ...
            if (cancelOrder != null) {
                rabbitTemplate.convertAndSend("order.cancel", GsonUtil.toJson(cancelOrder));
            }
            return;
        }
        // 当前单为委托买单
        if (order.getType() == 1) {
            ...
            // 异步资金清算
            rabbitTemplate.convertAndSend("order.tx", GsonUtil.toJson(tx));
