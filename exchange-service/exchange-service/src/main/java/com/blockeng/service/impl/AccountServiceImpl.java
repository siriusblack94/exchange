...
//        QueryWrapper<Account> accountEx = new QueryWrapper<>();
//
//        accountEx.eq("user_id", userId);

        List<AssetCountDTO> accountList=baseMapper.countAssets(userId);

        //筛选后能拿出去展示的List
        List<AssetCountDTO> accountResult=new ArrayList<AssetCountDTO>();

        for (AssetCountDTO assetCountDTO:accountList){

            Long coinId = assetCountDTO.getCoinId();
            BigDecimal volume=assetCountDTO.getBalanceAmount().add(assetCountDTO.getFreezeAmount());
            if (assetCountDTO.getCoinName()==null||assetCountDTO.getCoinStatus() != BaseStatus.EFFECTIVE.getCode()) {
                // 过滤无效币种
                continue;
            }
            assetCountDTO.setCarryingAmount(assetCountDTO.getBalanceAmount().add(assetCountDTO.getFreezeAmount()));

            accountResult.add(assetCountDTO);
            ...
            if (assetCountDTO.getCoinId().equals(baseCoinId)) {
        ...
        map.put("assertList", accountResult);
        return map;
